.include "shell_clear_address.s"
#########################################################################
# Rotina de tratamento de excecao e interrupcao		v2.6		#
...
# Usar o RARS16_Custom3							#
# Selecionar o Memory Configuration Default, Text at Kernetl		#
# Dump Memory   MIF32 K format para gerar_kdata.mif e _ktext.mif	#
#########################################################################


#########################################################################
# Defini��e e Macros						v2.6	#
...
.data   # na verdade ser� colocado no .kdata
.align 2
...
Addrs:  		.string "Addrs: "
Instr:  		.string "Instr: "
press_key_msg: .string "Pressione qualquer tecla para continuar"
...
ecallException:   addi    sp, sp, -264              # Salva todos os registradores na pilha
 ...
 ecallException.pula:   
	...
	goToExit: li t0, 0xFF200604
      ...
      jal readChar
      addi   sp, sp, 264
      li t0, SHELL_KTEXT_CLEAR_ADDRESS 
      csrw 	t0, uepc	# coloca no registrador uepc
      uret
  ...
  goToOpen: jal Open                        # chama Open
      j       endEcall

  goToClose: jal Close                      # chama Close
      j       endEcall

  goToLSeek: jal LSeek                      # chama LSeek
      j       endEcall

  goToRead: jal Read                        # chama Read
      j       endEcall

  goToWrite: jal Write                      # chama Write
      j       endEcall
...
endEcall.pula:	#addi    sp, sp, 264
		...
		addi   sp, sp, 264	# libera espa�o da pilha
...
#####################################
# Retornos                          #
# a0 = num de caracteres digitados  #
# a1 = end do ultimo char           #
#####################################
...
# adiciona char no buffer e incrementa posicao
		beq a6, s1, iReadString.loop	# caso tenha o maximo de caracteres, ignora
		add t0, s0, a6	# posicao no buffer
		sb a0, 0(t0)		# escreve char no buffer

# imprime char
		slli t1, a6, 3	# calcula offset do x do char
		add a1, t1, s2	# calcula x do char
		mv a2, s3				# mesmo y
		mv a3, s4				# mesma cor
		mv a4, a5				# mesma frame
		jal printChar		# imprime char

		addi a6, a6, 1	# incrementa contador de caracteres

		mv a0, s4
		jal printCursor	# imprime cursor na proxima posicao

		j iReadString.loop	# loop para ler o proximo char

iReadString.backspace: beqz a6, iReadString.loop # se nao tem char, pula
		add t0, s0, a6 # posicao no buffer
...
# apaga o cursor
		...
		addi a6, a6, -1	# decrementa contador de caracteres
		mv a0, s4				# cor
...
# apaga o cursor
iReadString.linefeed: mv a0, zero			# cor = 0 (tudo preto)
		jal printCursor	# imprime cursor com tudo preto (apaga cursor antigo)
j iReadString.end

iReadString.end: add a1, s0, a6	# a1 = endereco do ultimo char
		sb zero, 0(a1)	# escreve \0 no buffer (fim da string)
		addi a1, a1, -1	# char antes do fim da string
    mv a0, a6
...
# a0 = cor
printCursor: beq a6, s1, printCursor.end	# ignora caso ja tenha escrito maximo de caracteres
		addi sp, sp, -4 # aloca espaco
		sw ra, 0(sp)		# salva ra
		mv a3, a0				# cor
		li a0, 0x5F			# ascii do underscore (_)
		slli a1, a6, 3	# calcula offset do x do cursor
...
RS232_SendInt.loop: slli t0, t2, 3  # i * 8
    srl a0, s0, t0  # a0 >> (i * 8)
    andi a0, a0, 0x00FF
    jal RS232_SendByte
    addi t2, t2, -1
...
RS232_SendBuf: addi sp, sp, -8
    ...
    beqz a1, RS232_SendBuf.end
RS232_SendBuf.loop: lb a0, 0(s0)
    jal RS232_SendByte
    addi s0, s0, 1
    blt s0, t2, RS232_SendBuf.loop

RS232_SendBuf.end: lw s0, 4(sp)
    lw ra, 0(sp)
    addi sp, sp, 8
    ret
...
RS232_SendString: addi sp, sp, -8
    ...
    mv s0, a0
RS232_SendString.loop: lb a0, 0(s0)
    jal RS232_SendByte
    beqz a0, RS232_SendString.end
    addi s0, s0, 1
    j RS232_SendString.loop

RS232_SendString.end: lw s0, 4(sp)
    lw ra, 0(sp)
    addi sp, sp, 8
    ret
...
RS232_ReadInt: addi sp, sp, -8
    ...
    mv s0, zero
    li t2, 3      # contador i (vai ate 0)
RS232_ReadInt.loop: jal RS232_ReadByte
    slli t0, t2, 3  # i * 8
    sll a0, a0, t0  # a0 << (i * 8)
    or s0, s0, a0
    addi t2, t2, -1
...
RS232_ReadBuf: addi sp, sp, -8
    ...
    mv s0, a0
    mv t2, zero   # contador
    beqz a1, RS232_ReadBuf.end
RS232_ReadBuf.loop: jal RS232_ReadByte
    sb a0, 0(s0)
    addi t2, t2, 1
    addi s0, s0, 1
    blt t2, s5, RS232_ReadBuf.loop

RS232_ReadBuf.end: lw s0, 4(sp)
    lw ra, 0(sp)
    addi sp, sp, 8
    ret
...
################################################
Open: DE1(s8, Open.DE1)
    ecall
    ret
Open.DE1: addi sp, sp, -4
    sw ra, 0(sp)  # salva endereco de retorno
...
#########################################
Close: DE1(s8, Close.DE1)
    ecall
    ret
Close.DE1: addi sp, sp, -4
    sw ra, 0(sp)
...
########################################
LSeek: DE1(s8, LSeek.DE1)
    ecall
    ret
LSeek.DE1: addi sp, sp, -4
    sw ra, 0(sp)
...
########################################
Write: DE1(s8, Write.DE1)
    ecall
    ret
Write.DE1: addi sp, sp, -4
    sw ra, 0(sp)
